...
int varrayc(varray_t*me, off_t i, void*pvalue){
	...
	if(i<0){
		...
		me->length -= i;
		memshift(me->point, me->length, -i*me->tsize);
		memcpy(me->point, pvalue, me->tsize);
	}else if(i>=me->length){
		if(tryresize(me,i+1))return -1;
		me->length = i+1;
		memcpy(varrayr(me,i), pvalue, me->tsize);
	}else{
		if(tryresize(me,me->length+1))return -1;
		++me->length;
		memshift(varrayr(me,i), me->length-i, me->tsize);
		memcpy(varrayr(me,i), pvalue, me->tsize);
	}
	return 0;
}
...
int varrayd(varray_t*me, off_t i, size_t length){
	...
	memshift(del, (me->length-=length)-i, -length*me->tsize);
	return 0;
}
